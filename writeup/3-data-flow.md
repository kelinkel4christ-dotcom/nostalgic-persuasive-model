...
| -------------- | ------------------------------- | ------------------------------------------------------------------- |
| **Process**    | Circle                          | A function that transforms input data into output data              |
...
The Level 1 DFD decomposes the system into seven main processes:
...
| **Output**     | Session Token returned to User; User Record written to D1 (User Store)             |
...
| Attribute      | Description                                                                                             |
| -------------- | ------------------------------------------------------------------------------------------------------- |
| **Input**      | Preferences (birth year, habit selection, genre preferences) from User                                  |
| **Processing** | Calculates the user's "Nostalgia Period" (birth year + 5 to birth year + 22); stores preference profile |
| **Output**     | Preference Record written to D1 (User Store)                                                            |
...
| Attribute      | Description                                                                     |
| -------------- | ------------------------------------------------------------------------------- |
| **Input**      | Daily Log (habit completion status, journal text) from User                     |
| **Processing** | Stores structured habit data; forwards journal text to Process 4.0 for analysis |
| **Output**     | Habit Log Record written to D4 (Log Store); Journal Text passed to Process 4.0  |
...
| **Output**     | Stress Score (0.0–1.0) and Emotion label passed to Process 5.0                                                      |
...
| Attribute      | Description                                                                                                                                                             |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Input**      | Stress Score and Emotion from Process 4.0; User Preferences from D1; Candidate Content from D2; Model Weights from D3                                                   |
...
| **Output**     | Recommendation (selected movie or song) returned to User                                                                                                                |

### Process 6.0: Process Feedbacks
...
| **Output**     | Weight Update written to D3 (Model Store); Feedback Record stored in D4 (Log Store)   |
...
| Attribute      | Description                                                                        |
| -------------- | ---------------------------------------------------------------------------------- |
| **Input**      | Request Report from Admin; User Profiles from D1; Aggregated Logs from D4          |
| **Processing** | Computes statistics (completion rates, group comparisons); formats data for export |
| **Output**     | Statistics and Data Export returned to Admin                                       |
...
| Request Report | Admin  | Process 7.0 | Request for statistics or data export          |
...
| User Preferences  | D1         | Process 4.0 | User context for text analysis                     |
...
| Model Weights     | D3         | Process 6.0 | Current weights for update calculation             |
